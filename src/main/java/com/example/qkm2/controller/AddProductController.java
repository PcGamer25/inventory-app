...
public class AddProductController implements Initializable {
    ...
    /**
     * This method adds the product to the inventory
     *
     * @param event
     */
    public void productSave(ActionEvent event) {
        if (dataCheck()) {
            int currentProductId = Inventory.genProductId();
            ...
            this.assParts.forEach(part -> Inventory.lookupProduct(currentProductId).addAssociatedPart(part));
            try {
                switchToMain(event);
            } catch (IOException e) {
